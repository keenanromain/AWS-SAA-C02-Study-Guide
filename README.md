...
- There are ***Standard Reserved Instances*** which are inflexible reservations that are discounted at 75% off of an On-Demand instance. Standard Reserved Instances cannot be moved between regions. You can choose if a Reserved Instance applies to either a specific Availability Zone, or an Entire Region, but you cannot change the region.
- There are ***Convertible Reserved Instances*** that are discounted at 54% off of On-Demand instances, but you can also modify the instance type at any point. For example, you suspect that after a few months your VM might need to change from a General Purpose VM to a Memory Optimized VM. So if you think that in the future you might need to change your VM type or upgrade your VMs capacity, choose Convertible Reserved Instances. There is no downgrading instance type with this option though.
- There are ***Scheduled Reserved Instances*** that are reserved according to a specified timeline that you set. For example, you might use Scheduled Reserved Instances if you run education software that only needs to be available during school hours. This option allows you to better match your needed capacity with a recurring schedule so that you can save money.
...
### EC2 Security
- With EC2, termination protection is disabled by default. This means that by default, you do not have a safe-guard in place of accidentally turning off your instance. You must turn this feature on if you want that extra bit of protection.
- Amazon EC2 uses public–key cryptography to encrypt and decrypt login information. Public–key cryptography uses a public key to encrypt a piece of data, such as a password, then the recipient uses the private key to decrypt the data. The public and private keys are known as a key pair.
- You can encrypt your root device volume which is where you install the OS. The SSD volume types for EC2 are limited to IOPS optimized, general purpose, and magnetic.
- By default, an EC2 instance with an attached Elastic Block Store (EBS) root volume will be deleted together when the instance is terminated. However, any additional or secondary EBS volume that is also attached will be preserved. This is because the root EBS volume is for OS installations and other low-level configuration settings. This setting can be modified, but it is usually easier to boot a new instance with a fresh root device volume.
- You can encrypt the root volume during creation time or with third-party tools like bit locker. Of course, additional or secondary EBS volumes are also encryptable as well.
