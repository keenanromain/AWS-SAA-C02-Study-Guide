...
- An EBS snapshot reflects the contents of a persistent disk during a concrete instant in time.
- An image (AMI) is the same thing, but includes an operating system and a boot loader so it can be used to boot an instance. 
- AMIs can be thought of as prebaked, launchable servers. AMIs are used to create an instance and were once created from an instance. 
- When you provision an EC2 instance, an AMI is actually the first thing you are asked to specify for the instance. You can choose a pre-made AMI or create your own from an EBS snapshot of a previously provisioned server.
